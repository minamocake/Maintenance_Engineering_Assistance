...
}else{var s1=escapeQuote(objectValue?objectValue.toString():objectValue);
var s2=escapeQuote(value);
toEval="'"+s1+"' "+op+" '"+s2+"'";
if(!isNaN(s1)&&!isNaN(s2)){toEval=s1+op+s2
}}var b=eval(toEval);
return b
}}}catch(e){console.log("Error in Operator resolution")
}return false
}}
}();
...
}m=CQ_Analytics.Utils.addSuffixToTeaserUrl(m);
...
if(SAP&&SAP.campaigns&&SAP.campaigns.valuePropMoveUp){q=SAP.campaigns.valuePropMoveUp(q)
}j(a,q);
delete CQ_Analytics.Utils.teasersLoading[a];
b();
window.picturefill();
if(window.targetBuildCarousel){window.targetBuildCarousel(".gallery-box")
}}}else{j(a,"")
}$(document).trigger("teaserLoaded")
})
};
...
}},addSuffixToTeaserUrl:function(a){if(CQ_Analytics.PageDataMgr.data&&CQ_Analytics.PageDataMgr.data.template){var c=CQ_Analytics.PageDataMgr.data.template;
c=c.substring(c.lastIndexOf("/")+1);
requestURL=CQ.shared.HTTP.addSelector(a,c)
}var b=decodeURIComponent($("html").attr("data-page-path"));
if(b){var d=a.lastIndexOf(".html"),e=b+".tandt.html";
if(d>0){d+=5;
a=a.substr(0,d)+e+a.substr(d)
}else{a+=e
}}return a
...
CQ_Analytics.JSONStore.registerNewInstance("demandbase",window._demandbaseInfo);
CQ_Analytics.JSONStore.registerNewInstance("user",{cmp_url_id:"",cmp_source:"",cmp_campaigncode:"",cmp_referrer:""})
...
r.push(o);
t=u
}}}}if(r.length>1){for(var p=0;
p<r.length;
p++){if(r[p].name==="default"){r.splice(p,1)
}}}return r
}var b={};
...
var s=function(){if(CQ_Analytics.CCM.clientcontext&&CQ_Analytics.CCM.clientcontext.demandbase){if(sessionStorage&&sessionStorage.getItem("demandbase_ip_widget")){CQ_Analytics.CCM.clientcontext.demandbase=JSON.parse(sessionStorage.getItem("demandbase_ip_widget"))
}}var J=null;
if(l()){J=[]
...
}if(CQ_Analytics.CCM.clientcontext&&(!CQ_Analytics.CCM.clientcontext.demandbase||Object.keys(CQ_Analytics.CCM.clientcontext.demandbase).length==0)){M=null
