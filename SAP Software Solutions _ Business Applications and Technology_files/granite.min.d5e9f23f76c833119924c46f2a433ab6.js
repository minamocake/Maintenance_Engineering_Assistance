...
}var p=":cq_csrf_token",g="CSRF-Token",k=m.externalize("/libs/granite/csrf/token.json");
var j,r;
...
XMLHttpRequest.prototype.open=function(u,s,t){if(e(s)&&u.toLowerCase()!=="get"){this._csrf=true;
this._async=t
...
c.apply(s,t)
},function(){if(window.console){console.error("Unable to read CSRF meta information")
}c.apply(s,t)
})
};
...
if(s.nodeName==="IFRAME"&&s.contentWindow&&!s._csrf){s._csrf=true;
try{i(s.contentWindow.document)
